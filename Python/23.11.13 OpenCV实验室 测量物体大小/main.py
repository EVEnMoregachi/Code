...
image = cv2.imread('fiximage3.jpg')
# 输入图片灰度化
...
# 在边缘图像中寻找物体轮廓 cv2.RETR_EXTERNAL 指的是只检索最外面的轮廓 并存储水平、垂直和对角线的终点
...
for c in cnts:
    ...
    box = cv2.cv.BoxPoints(box) if imutils.is_cv2() else cv2.boxPoints(box) #将最小外接矩形转换为角点坐标的列表
    box = np.array(box, dtype="int") #将角点坐标列表转换为numpy数组
    ...
    if pixelsPerMetric is None:
        pixelsPerMetric = dB / 30 # 设置已知物体的实际宽度 (单位mm)

    # 计算目标的实际大小（宽和高），用mm来表示
