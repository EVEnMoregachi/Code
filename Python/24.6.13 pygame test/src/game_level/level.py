...
class Level:
    def __init__(self, name, level_map_file, space, surface):
        ...
        # 加载背景图像
        self.background_image = pygame.image.load("./src/resources/images/level/" + self.background_file_name).convert()
        ...
        # 加载音频文件
        self.sound_handle_sling = pygame.mixer.Sound("./src/resources/sounds/slingshot streched.wav")
        self.sound_bird_flay = pygame.mixer.Sound("./src/resources/sounds/bird 01 flying.wav")
        self.sound_bird_short = pygame.mixer.Sound("./src/resources/sounds/bird shot-a1.wav")
        self.sound_level_clear = pygame.mixer.Sound("./src/resources/sounds/level clear military a1.wav")
        self.sound_level_failed = pygame.mixer.Sound("./src/resources/sounds/level failed piglets a1.wav")
    ...
    def send_pm_bird(self, mouse_x, mouse_y):
        self.sound_bird_flay.play() 
        self.sound_bird_short.play()
        ...
        bird = Bird(self.current_standby_bird.bird_info, self.current_standby_bird.pos_x, self.current_standby_bird.pos_y,
                    Vec2d((pm_sling_x - pm_x) * 3, (pm_sling_y - pm_y) * 3), self.space, self.surface)
    ...
    def run(self):
        ...
        if len(self.pig_group) == 0 or len(self.bird_group) == 0:
            self.game_over()
    ...
    def game_over(self):
        if self.timeflag == False:
            ...
            self.timeflag = True
        if time.time() - self.trigger_time >= 3:
            if len(self.pig_group) == 0:
                self.Level_type = "clear"
            elif len(self.bird_group) == 0:
                self.Level_type = "failed"
            if self.Level_type == "clear" and not self.sound_played:
                self.sound_played = True
                self.sound_level_clear.play()
            elif self.Level_type == "failed" and not self.sound_played:
                self.sound_played = True
                self.sound_level_failed.play()
        if time.time() - self.trigger_time >= 6:
            self.status = 'end'
    ...
    def handel_sling(self, mouse_pressed, mouse_up):
        ...
        if mouse_pressed and self.current_standby_bird is not None:
            if mouse_x > self.current_standby_bird.pos_x\
                ...
                self.sound_handle_sling.play()
                self.sling_click = True
        elif self.sling_click:
            self.current_standby_bird.pos_x, self.current_standby_bird.pos_y = mouse_x - 17, mouse_y - 17
            if mouse_up:
                self.sling_click = False
